                    ...
                    <tr>
                        ...
                        <td>
                            @if ($peminjaman->status == 'pending')
                                ...
                                <form action="{{ route('admin.reject', ['id' => $peminjaman->peminjaman_id]) }}"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form id="approveForm_{{ $peminjaman->peminjaman_id }}"
                        action="{{ route('admin.approve', ['id' => $peminjaman->peminjaman_id]) }}" method="POST">
                        @csrf
                        <div class="modal-body">
                            ...
                            <input type="date" class="form-control"
                                id="tanggal_pengembalian_{{ $peminjaman->peminjaman_id }}" name="tanggal_pengembalian"
                                required>
                        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form id="returnForm_{{ $peminjaman->peminjaman_id }}"
                        action="{{ route('admin.return', ['id' => $peminjaman->peminjaman_id]) }}" method="POST">
                        @csrf
                        <div class="modal-body">
                            <input type="hidden" name="peminjaman_id" value="{{ $peminjaman->peminjaman_id }}">
                            <label for="kondisi_buku_{{ $peminjaman->peminjaman_id }}" class="form-label">Kondisi
                                Buku</label>
                            ...
                            <div class="mt-3 d-none" id="telatInfo"> <!-- Hidden by default -->
                                <label for="jumlah_hari_telat_{{ $peminjaman->peminjaman_id }}" class="form-label">Jumlah
                                    Hari Telat</label>
                                <input type="number" class="form-control"
                                    id="jumlah_hari_telat_{{ $peminjaman->peminjaman_id }}" name="jumlah_hari_telat">
                            </div>
                            <div class="mt-3">
                                <label for="denda_{{ $peminjaman->peminjaman_id }}" class="form-label">Denda</label>
                                <input type="text" class="form-control" id="denda_{{ $peminjaman->peminjaman_id }}"
                                    name="denda" readonly>
                            </div>
                            <input type="hidden" class="form-control tanggal-pengembalian"
                                id="tanggal_pengembalian_{{ $peminjaman->peminjaman_id }}"
        ...
        $(document).ready(function() {
            ...
            $('.kondisi-buku').on('change', function() {
                ...
                function formatRupiah(angka) {
                    var rupiah = 'Rp ' + angka.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
                    return rupiah.replace('.00', ',00');
                }
                ...
                dendaInput.val(formatRupiah(denda));
            });
